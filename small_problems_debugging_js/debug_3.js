/* JS110 Small Problems Debugging JS 3:

Problem: One bored and hungry evening we decided to randomly generate recipes. We 
can't wait to see the first suggestions, but JavaScript raises a TypeError, telling 
us that dishName.join is not a function. What is wrong?

Answer: when `dishName` and `dish` are declared, they're assigned to the result of
attempting to concatenate arrays using the string concatenation operator. this operation
coerces the arrays to strings, so calling the `join` method on `dishName` and `dish`
will raise an error -- `join` is an array method.
*/
